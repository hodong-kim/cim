...
/*
 * A statically linked GNAT stand-alone library is not guaranteed to initialize
 * itself on every platform. These symbols are generated by the SAL binder.
 */
extern void ciminit  (void);
extern void cimfinal (void);

int
test_initialize_cim_runtime (void)
{
  ciminit ();

  if (atexit (cimfinal) != 0)
  {
    cimfinal ();
    fprintf (stderr,
             "test_initialize_cim_runtime: failed to register finalizer\n");
    return 1;
  }
